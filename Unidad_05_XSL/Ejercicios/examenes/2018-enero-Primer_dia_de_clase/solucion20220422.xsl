			...
			<xsl:for-each select="hora">				
				...
				<xsl:choose>
					<xsl:when test="position() mod 2 = 1">
						<xsl:call-template name="horaEnDia">
							<xsl:with-param name="color">lightblue;</xsl:with-param>
						</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="horaEnDia">
		...
		<td>
			<xsl:for-each select="/primerDia1DAW/estructuraAula/bloque">
				<xsl:if test="@x = $x and @y = $y">
					...
					<xsl:attribute name="style">background:#<xsl:value-of select="/primerDia1DAW/estructuraAula/color[@tipo = $tipoDelBloque]"/>;</xsl:attribute>
